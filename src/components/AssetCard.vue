<template>
  <v-card>
    ...
    <v-chip v-if="cardData.fanmade">Fanmade Asset</v-chip>
    <v-card-actions>
      <v-btn flat v-for="(downloadCardData,i) in cardData.formats" :key="i" :href="downloadCardData.link" target="_blank">
        {{downloadCardData.format}}
      </v-btn>
      <v-btn flat v-if="cardData.colour" @click.stop.prevent="copyColour">
        {{cardData.colour}}
